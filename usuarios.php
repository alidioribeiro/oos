...
if(!empty($pesq))
{
	$sql   = "SELECT * FROM usuarios WHERE login LIKE '%$pesq%' AND EXCLUIR='' ORDER BY nome";
}
else
{
	$sql   = "SELECT * FROM usuarios WHERE EXCLUIR='' ORDER BY nome";
}
...
if($query)
{
	...
	while ($linha = mysql_fetch_row($query))
	{
		if ($marcador >= $linha_inicial and $marcador <= $linha_final)
		{
			...
			$setor   = $linha[3];
			$admin   = $linha[5];
		echo "<tr id=".$marcador." onmouseover='selecionado(".$marcador.")' onmouseout='deselecionado(".$marcador.")'><td>".$nome."</td><td>".$login."</td><td>".$setor."</td><td><img src='drop.png' onclick=excluir('".$linha[0]."') title='clique para excluir o registro'></td></tr>";
		 }
			$marcador++;
	}
}
